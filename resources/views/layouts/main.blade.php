    ...
    <nav id="sidebar" class="w-20 h-full bg-zinc-200 transition-all ease-linear duration-300 flex flex-col fixed z-10">
        <div class="w-full h-1/6 items-center justify-center flex flex-col">
            ...
            <div class="w-20 h-20 rounded-full flex justify-center items-center p-2">
                <img src="{{asset('storage/'. Auth::user()->image)}}" alt="" class="w-[rem3] h-full rounded-full">
            </div>
            <div class="">
        ...
        <div class="w-full h-4/6">
            <ul class="whitespace-nowrap overflow-ellipsis h-full flex flex-col gap-2 p-2">
                ...
                <li><a href="{{route('other_users')}}" class="gap-2 overflow-hidden hover:bg-gray-300 flex items-center text-lg py-3 rounded-lg transition-all duration-300 ease-linear"><i class="px-5 ml-0.5 fa-solid fa-users"></i>Explorar</a></li>
                @endauth
                @guest
                <li><a href="{{route('login')}}" class="gap-2 overflow-hidden hover:bg-gray-300 flex items-center text-lg py-3 rounded-lg transition-all duration-300 ease-linear"><i class="px-5 ml-0.5 fa-solid fa-arrow-right-to-bracket"></i>Entrar</a></li>
        ...
        <div class="w-full h-1/6 whitespace-nowrap overflow-ellipsis flex flex-col gap-2 p-2">
            ...
            <form action="{{route('logout')}}" method="post">
                @csrf
                <a href="" class="gap-2 overflow-hidden flex items-center text-lg py-3 rounded-lg transition-all duration-300 ease-linear hover:bg-gray-300" onclick="event.preventDefault(); this.closest('form').submit();"><i class="fa-solid fa-arrow-right-from-bracket px-5 ml-0.5"></i>Sair</a>
            </form>
            @endauth
        </div>
