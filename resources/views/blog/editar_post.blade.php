        ...
        <form action="{{route('editar_post', $post->id)}}" method="post" enctype="multipart/form-data" class="w-[80%] h-full">
            ...
            xl:h-[40%]">
                <div class="w-full h-full random-color">
                    <img src="{{asset('storage/' . $post->image_post)}}" alt="" class="w-full h-full hidden" id="image-preview">
                </div>
            ...
            <div class="w-full h-[80%]">
                ...
                sm:justify-end">
                    ...
                    <div class="w-2/3 h-full ">
                        <div class="w-full h-1/2 flex items-end">
                            <h4 class="text-xl">Autor: {{$user->name}}</h4>
                ...
                <div class="w-full h-[10%] items-center justify-evenly hidden">
                    ...
                    <div class="w-full h-[90%] flex flex-col justify-center gap-3 items-center">
                        ...
                        <input type="text" name="title" id="title" class="w-4/5 h-10 rounded-md bold border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 resize-none" value="{{$post->title}}" placeholder="Título...">
                        @error('title')
                        <div class="text-red-600 w-4/5 mb-4">{{ $message }}</div>
                        @enderror

                        <textarea name="description" id="description" placeholder="Descrição..." class="w-4/5 h-[12rem] border-sky-400 rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 resize-none" value="{{old('description')}}">
                        {{$post->description}}
                        </textarea>
                        @error('description')
                        <div class="text-red-600 w-4/5 mb-4">{{ $message }}</div>
                        ...
                        <textarea name="content" id="content" placeholder="Conteúdo..." class="w-4/5 h-[28rem] border-sky-400 rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 resize-none">
                        {{$post->content}}
                        </textarea>
                        @error('content')
...
@endsection
@section('js')
{{asset('js/script_edicao_do_post.js')}}
@endsection