...
class PostController extends Controller
{
    ...
    public function tela_editar_post($id) //Função para mostrar a view de editar o post
    {
        ...
        $user = User::findOrFail($post->user_id);
        if ($post && $user) {
            return view("blog.editar_post", ["post" => $post, "user" => $user]);
        } else {
    ...
    public function search_users(Request $request) //Função para buscar os usuarios
    {
        $data = $request->except('_token', 'submit');
        $search = $data['search'];
        $users = User::where('name', 'like', $search . "%")->orderBy("created_at")->paginate(20);
        $adminEmail = 'admin@admin';
        if (count($users) == 0) {
            return redirect()->back()->with("error", "Nenhum resultado encontrado");
        } else {
            return view('blog.explorar', ['users' => $users, 'search' => $search, 'adminEmail' => $adminEmail]);
        }
    }
    public function dash_search(Request $request) //Função para buscar os usuarios (dashboard)
    {
        $data = $request->except('_token', 'submit');
        $search = $data['search'];
        $users = User::all();
        $posts = Post::where('title', 'like', "%" . $search . "%")->orderBy("created_at")->paginate(6);
        if (count($posts) == 0) {
            return redirect()->back()->with("error", "Nenhum resultado encontrado");
        } else {
            return view('blog.explorar', ['posts' => $posts, 'search' => $search, 'users' => $users]);
        }
    }
    ...
    public function atualizar_perfil(Request $request, $id)
    {
        ...
        if ($request->hasFile('image') && $request->file('image')->isValid()) {
            if (
                $perfil->image != NULL && Storage::disk('public')->exists($perfil->image) && $perfil->image != 'images/fixed/guestUser.jpg'
            ) {
                if (Storage::disk('public')->exists($perfil->image) && $perfil->image != 'images/users/seed_user/ImagemDoADM.jpg')
